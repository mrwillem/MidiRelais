...
/* Configure the LCD_SPI interface ----------------------------------------------*/
  ...
  else if(LCDType == LCD_ILI9341)
  {
	  /* Set Display Reset */
	  //LCD_CtrlLinesWrite(LCD_SPI_MISO_PIN, LCD_SPI_MISO_PORT, Bit_SET);
	  //LCD_CtrlLinesWrite(LCD_SPI_MOSI_PIN, LCD_SPI_MOSI_PORT, Bit_SET);
...
static void LCD_WriteRegILI9341(uint8_t LCD_Reg)
{
  ...
  // LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NWR_PIN, Bit_RESET);

  LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NCS_PIN, Bit_RESET);

  //tmp = LCD_Reg << 8;
  //tmp |= LCD_Reg;
  //PseudoSPI_SEND(LCD_Reg);
  SPI_I2S_SendData(LCD_SPI, LCD_Reg);

  while(SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_BSY) != RESET)
  {
  }
  // LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NWR_PIN, Bit_SET);
  LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NCS_PIN, Bit_SET);
...
void LCD_WriteRAM(uint16_t RGB_Code)
{
  ...
  if(LCDType == LCD_ILI9341)
  {
	  ...
	  //PseudoSPI_SEND((uint8_t)RGB_Code);
	  // LCD_CtrlLinesWrite(LCD_NWR_GPIO_PORT, LCD_NWR_PIN, Bit_SET);
	  LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NCS_PIN, Bit_SET);
  }
...
/**
  * @}
  */
void PseudoSPI_SEND(uint8_t datain)
{
  char data;
  data=(uint8_t)datain;
  unsigned char c = 0;
  ...
  {
    if (data & 0x80)
    	LCD_CtrlLinesWrite(LCD_NWR_GPIO_PORT, LCD_SPI_MOSI_PIN, Bit_SET);
    else
    	LCD_CtrlLinesWrite(LCD_NWR_GPIO_PORT, LCD_SPI_MOSI_PIN, Bit_RESET);
    data = data<<1;
    LCD_CtrlLinesWrite(LCD_NWR_GPIO_PORT, LCD_SPI_SCK_PIN, Bit_RESET);
  asm ("nop");
    LCD_CtrlLinesWrite(LCD_NWR_GPIO_PORT, LCD_SPI_SCK_PIN, Bit_SET);
  }
  // P1OUT |= LCD_CS;
}
